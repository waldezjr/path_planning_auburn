...
void draw_robot(robot r,IplImage* img){
                ...
                phi1=r.r_phi1();
                phi2=r.r_phi2();
                double b1=(float) phi1/15*255;
                double b2=(float) phi2/15*255;


                cvFloodFill( img, center1, CV_RGB(b2,0,0), cvScalarAll(0), cvScalarAll(0), NULL, 0, NULL );
                cvFloodFill( img, center2, CV_RGB(b1,0,0), cvScalarAll(0), cvScalarAll(0), NULL, 0, NULL );
...
void draw_obs(obstacle o,IplImage* img){

        cvCircle(img,o.r_pos(),20,CV_RGB(255,255,255),-1);
        CvFont font;
                ...
                char buffer[1];
                sprintf(buffer,"%d",o.r_id());
                cvPutText (img,buffer,cvPoint(o.r_pos().x-10,o.r_pos().y+10), &font, CV_RGB(255,0,255));
}

void draw_goal(goal g,IplImage* img){

        typedef struct color_tab color;
    struct color_tab{
        int r;
        int g;
        int b;
    ...
    int i=g.r_cluster();
        ...
        cvCircle(img,g.r_pos(),25,CV_RGB(cor[i].r,cor[i].g,cor[i].b),-1);
        cvCircle(img,g.r_pos(),20,CV_RGB(255,0,0),-1);
        cvCircle(img,g.r_pos(),15,CV_RGB(255,255,0),-1);
        cvCircle(img,g.r_pos(),10,CV_RGB(255,0,0),-1);
        cvCircle(img,g.r_pos(),5,CV_RGB(255,255,0),-1);
        }
        else{
        cvCircle(img,g.r_pos(),25,CV_RGB(cor[i].r,cor[i].g,cor[i].b),-1);
        cvCircle(img,g.r_pos(),20,CV_RGB(0,0,255),-1);
        cvCircle(img,g.r_pos(),15,CV_RGB(0,255,255),-1);
        cvCircle(img,g.r_pos(),10,CV_RGB(0,0,255),-1);
        cvCircle(img,g.r_pos(),5,CV_RGB(0,255,255),-1);
        }
                CvFont font;
                int    lineWidth=2;
                cvInitFont(&font,CV_FONT_HERSHEY_SIMPLEX|CV_FONT_ITALIC, 1,1,0,lineWidth);
                ...
                char buffer[1];
                sprintf(buffer,"%d",g.r_id());
                cvPutText (img,buffer,cvPoint(g.r_pos().x-10,g.r_pos().y+10), &font, CV_RGB(0,0,0));
}
